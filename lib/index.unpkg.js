...
const handleAfterRender = (afterRender, output, startTime) => {
  if (typeof afterRender === "function") {
    afterRender({ ...output, stay: new Date() - startTime });
  }
};
  ...
  const changeRouter = (0,external_root_React_commonjs_react_commonjs2_react_amd_react_.useCallback)(({ inputPath = "", inputParams = {} }, historyCallback) => {
    ...
    new Promise((resolve, reject) => {
      ...
      if (inputPath !== prevPathRef.current && typeof beforeRender === "function") {
        beforeRender({ path: inputPath, params: inputParams, prevPath: prevPathRef.current }, resolve, reject);
      } else {
        resolve();
      }
    }).then((res) => {
      ...
      if (redirect) {
        ...
        push({ path: redirect, exact: redirect !== "/404" });
        return;
      }
...
// EXTERNAL MODULE: ../../node_modules/.pnpm/css-loader@6.7.3_webpack@5.75.0/node_modules/css-loader/dist/cjs.js??ruleSet[1].rules[2].use[1]!../../node_modules/.pnpm/less-loader@11.1.0_less@4.1.3+webpack@5.75.0/node_modules/less-loader/dist/cjs.js??ruleSet[1].rules[2].use[2]!../huxy/router/link.less
var router_link = __webpack_require__(982);
...
const fixClassName = (className) => {
  ...
  const activeIndex = cls.findIndex((c) => c === "active");
  if (activeIndex > -1) {
    cls.splice(activeIndex, 1);
    return [huxy_router_link.link, huxy_router_link.active, ...cls].join(" ");
  }
  return [...cls, huxy_router_link.link].join(" ");
};
const Link = (props) => {

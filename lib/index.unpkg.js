...
const loadComps_excluded = ["current", "menu"],
      loadComps_excluded2 = ["component", "resolve", "loadData", "loading", "errorBoundary"],
...
const loadComps = props => {
  ...
  const tempCurrent = [...current].filter(item => item.component); // const compsLength = tempCurrent.length;
  // reset memo
  ...
  const loader = function () {
    ...
    if (typeof component === 'function' || utils_isAsync(component)) {
      ...
      let Comp = cacheLoaded[compKey]; // const canUseMemo = compLevel !== compsLength - 1;
      // get memo
      ...
      if (utils_isAsync(Comp)) {
        return Comp.then(res => {
          ...
          cacheLoaded[compKey] = Comp; // canUseMemo && setMemo(compKey, Comp);
...
const useRouter = initState => {
  ...
  const changeRouter = (0,external_root_React_commonjs_react_commonjs2_react_amd_react_.useCallback)((_ref, historyCallback) => {
    ...
    new Promise((resolve, reject) => {
      ...
      historyCallback == null ? void 0 : historyCallback(); // reset scroll
      ...
      const components = src_loadComps(componentProps); // reset initPath

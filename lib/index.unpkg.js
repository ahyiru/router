...
__webpack_require__.d(__webpack_exports__, {
  ...
  "routeStore": function() { return /* reexport */ routeStore; },
  "useRoute": function() { return /* reexport */ useRoute; },
...
function formatMenu_toPropertyKey(arg) { var key = formatMenu_toPrimitive(arg, "string"); return typeof key === "symbol" ? key : String(key); }
function formatMenu_toPrimitive(input, hint) { if (typeof input !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (typeof res !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
    ...
    for (let i = 0, l = cdata.length; i < l; i++) {
      ...
      if (item[idKey] === pid) {
        ...
        const {
          ...
          restItem = _objectWithoutProperties(item, [childKey].map(formatMenu_toPropertyKey));
...
const handleAfterRender = (afterRender, output, startTime) => {
  if (typeof afterRender === 'function') {
    afterRender(utils_objectSpread(utils_objectSpread({}, output), {}, {
      stay: new Date() - startTime
    }));
  }
};
...
var spinner = __webpack_require__(435);
...
const LoadCompData = props => {
  ...
  return /*#__PURE__*/(0,jsx_runtime.jsx)(Comp, loadCompData_objectSpread(loadCompData_objectSpread(loadCompData_objectSpread(loadCompData_objectSpread({}, params), resolveResult), loadResult), {}, {
    children: children
  }));
};
...
const _excluded = ["params"],
  _excluded2 = ["loading", "errorBoundary"];
...
const wrapComponent = _ref2 => {
  ...
  const {
    ...
    restProps = _objectWithoutProperties(routerProps, _excluded);
  const {
      loading: Loading,
...
const loadComps_excluded = ["current", "menu"],
  loadComps_excluded2 = ["component", "resolve", "loadData", "loading", "errorBoundary", "icon"],
  _excluded3 = ["component", "resolve", "loadData", "icon", "key"];
...
// const {setMemo, getMemo, resetMemo} = memoComponents();
...
const handleChild = (Comp, routerProps, children) => {
  if (utils_isAsync(children)) {
    return children.then(res => {
      var _res$default;
      return src_wrapComponent({
        Comp,
        routerProps,
...
const loadComps = props => {
  const {
    ...
    restProps = _objectWithoutProperties(props, loadComps_excluded);
  const currentProps = current.map(propItem => {
    const {
      ...
      restItem = _objectWithoutProperties(propItem, loadComps_excluded2);
    return restItem;
  });
  const tempCurrent = [...current].filter(item => item.component);
  // const compsLength = tempCurrent.length;
  // reset memo
  // resetMemo(tempCurrent.slice(0, -1));
  const loader = function () {
    ...
    if (typeof component === 'function' || utils_isAsync(component)) {
      ...
      const compKey = restItemProps.path; // JSON.stringify(restItemProps);
      let Comp = cacheLoaded[compKey];
      // const canUseMemo = compLevel !== compsLength - 1;
      // get memo
      // canUseMemo && (Comp = getMemo(compKey) ?? Comp);
      // get cached data
      const {
        cachedResolve,
        restResolve
      } = getCachedResolve(resolve, restProps.store, restItemProps);
      // get loaded
      ...
      const routerProps = {
        ...
        }, restProps), restItemProps), cachedResolve)
      };
      // loader children
      const children = loader(compLevel + 1);
      if (Comp) {
        return handleChild(Comp, routerProps, children);
      }
      try {
        Comp = component();
      ...
      if (utils_isAsync(Comp)) {
        return Comp.then(res => {
          ...
          // canUseMemo && setMemo(compKey, Comp);
          return handleChild(Comp, routerProps, children);
        });
      }
      cacheLoaded[compKey] = Comp;
...
const router_excluded = ["browserRouter", "childKey", "idKey", "title", "routers", "beforeRender", "afterRender", "basepath", "allowedNotFound"],
...
const useRouter = initState => {
  ...
  const historyEvents = function (opt) {
    let event = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'pushState';
    const {
      basepath,
      browserRouter
    } = inputRef.current;
    // const localPath=getPath(browserRouter).path;
  ...
  const changeRouter = (0,external_root_React_commonjs_react_commonjs2_react_amd_react_.useCallback)((_ref, historyCallback) => {
    let {
      inputPath = '',
      inputParams = {}
    } = _ref;
    ...
    const _inputRef$current = inputRef.current,
      ...
      restInput = _objectWithoutProperties(_inputRef$current, router_excluded);
    // get router
    if (!inputPath) {
      const {
        path,
    ...
    new Promise((resolve, reject) => {
      if (!(routers != null && routers.length)) {
        reject('路由为空！');
      }
      ...
      inputPathRef.current = (_inputPathRef$current = inputPathRef.current) != null ? _inputPathRef$current : inputPath;
      // handle routers && redirect
      const {
        redirect,
        current,
        menu
      ...
      if (redirect) {
        if (allowedNotFound && redirect === '/404') {
      ...
      // inject some data
      routeStore.setState(router_objectSpread(router_objectSpread({}, componentProps), {}, {
        browserRouter
      }));
      // load components
...
const useRoute = () => routeStore.getState();

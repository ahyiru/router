...
const fixClassName = (propCls, defCls) => [.../* @__PURE__ */ new Set([...(defCls != null ? defCls : "").trim().split(" "), ...(propCls != null ? propCls : "").trim().split(" ")])].filter(Boolean).join(" ");
const Link = (props) => {
  const { to, onClick, preventDefault, stopPropagation, exact, target, className, style, ...rest } = props;
  return /* @__PURE__ */ (0,jsx_runtime.jsx)("span", { onClick: src_handleLinkClick(props), className: fixClassName(className, "link"), style: { ...linkStyle, style }, ...rest });
};
